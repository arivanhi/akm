...
file_path = 'data/adc_asam_urat_v3.csv'
df = pd.read_csv(file_path)
X_train = df.iloc[:, 0:120].to_numpy()
X_train = np.round(X_train).astype(int)
# X_train = np.transpose(X_train)
y_train = df.iloc[:, 120:].to_numpy()
...
load_model_path = 'model/xgboost_as2_model.json'
...
load_model_path = 'model/knn_as2_predictions.npy'
